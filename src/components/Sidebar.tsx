...
function Sidebar({ setShow }: SidebarProps) {
  ...
  const [selectedProfile, setSelectedProfile] =
    useState<keyof typeof toolsByProfile>("developer");
  const handleClose = () => {
    setShow(false);
  };
>>>>>>> Stashed changes
    ...
    <>
      <AnimatePresence>
        <motion.div
          ...
          ref={sidebarRef}
          className="h-screen w-[20vw] absolute left-0 top-0 bg-white/50 backdrop-blur-2xl rounded-lg shadow-lg z-30 "
        >
          <div className="p-4 pt-10">
            <label htmlFor="profile-select" className="text-gray-100">
              Select Profile:
            </label>
            <select
              id="profile-select"
              ...
              onChange={(e) =>
                setSelectedProfile(
                  e.target.value as keyof typeof toolsByProfile
                )
              }
            >
          ...
          <div className="p-4">
            <ul className="text-white list-none flex flex-col gap-2 justify-evenly ">
              {toolsByProfile[selectedProfile].map((tool) => (
                <li
                  key={tool.id}
                  onClick={() => updateMainLayout(tool.id)}
          ...
          <div className="absolute right-1 top-1" onClick={handleClose}>
            {" "}
            <CircleX />
          </div>
        </motion.div>
