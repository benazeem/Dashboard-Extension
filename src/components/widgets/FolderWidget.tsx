...
function FolderWidget({ FolderProps }: FolderWidgetProps) {
  ...
  const [folderName, setFolderName] = useState(name);
  const [hoverPreviewItemIcon, setHoverPreviewItemIcon] = useState<
    string | null
  >(null);
  const sites = useSelector(getAllSites);
>>>>>>> Stashed changes
      ...
      <div className="icon-container relative flex items-center justify-center  rounded-md container40 ">
        <div className="absolute top-0 left-0 w-full h-full ">
          ...
          <div className="app-folder-preview h-full w-full overflow-hidden">
            ...
            {iconList? (iconList.map((icon: string) => (
              <div className="w-full h-full flex items-center justify-center overflow-hidden rounded-xs" key={crypto.getRandomValues(new Uint32Array(1))[0]}>
                <div className="w-full h-full relative rounded-[30%]">
                 <div className="w-full h-full flex justify-center bg-cover bg-no-repeat bg-top-left" style={{ backgroundImage: `url(${icon})`}}>
                 </div></div> </div>
            ))):(<>
              {itemIcons?.map((icon) => (
                <div className="w-full h-full flex items-center justify-center overflow-hidden rounded-xs" key={crypto.getRandomValues(new Uint32Array(1))[0]}>
                <div className="w-full h-full relative rounded-[30%]">
               <div className="w-full h-full" style={{ backgroundImage: `url(${icon})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
                </div></div></div> ))}
                {hoverPreviewItemIcon ? (
                  <div className="w-full h-full flex items-center justify-center overflow-hidden rounded-xs" key={crypto.getRandomValues(new Uint32Array(1))[0]}>
                  <div className="w-full h-full relative rounded-[30%]">
                  <div className="w-full h-full" style={{ backgroundImage: `url(${hoverPreviewItemIcon})`, backgroundSize: 'cover', backgroundPosition: 'center' }}></div>
                  </div></div> ) : null }
                </>
                )}
</div>
                  ...
                  {iconList ? (
                    iconList.map((icon: string) => (
                      ...
                      >
                        <div className="w-full h-full relative rounded-[30%]">
                          <div
                            className="w-full h-full flex justify-center bg-cover bg-no-repeat bg-top-left"
