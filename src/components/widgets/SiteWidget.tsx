...
function SiteWidget({ icon, url, name,parent, id}: SiteWidgetType) {
  ...
  const style : React.CSSProperties = parent==='dock'?{
  ...
  const handleSiteOverSite = (event: DragOverEvent) => {
    ...
    if (activeId.startsWith('bookmark-')) {
      ...
      const overItemIcon = sites.find((site) => site.id === overId.slice(5))?.icon;
      if (activeItemIcon && overItemIcon) {
        setMergerPreview({iconList:[activeItemIcon, overItemIcon], id: overId.slice(5)});
        ;
      }else{
        setMergerPreview({iconList: [icon, icon], id: null});
      }
  ...
  return (
    ...
    <DndMonitor
      ...
      {(mergePreview.id && mergePreview.id === id)? (
                <FolderWidget key={crypto.getRandomValues(new Uint32Array(1))[0]} FolderProps={{ iconList: mergePreview.iconList}} />):(
                  <>
      <a title={name} href={url} className="h-full w-full flex items-center justify-center flex-col overflow-hidden box-border animate-wrapper">
      
      <div className={`icon-container relative flex items-center justify-center ${parent==='dock'?'':'container40'}`} style={style}>
        <div className="absolute top-0 left-0 w-full h-full">
            <div className="flex items-center justify-center overflow-hidden w-full h-full rounded-md">
                <div className="w-full h-full" style={{ backgroundImage: `url(${icon})`, backgroundSize: 'cover', backgroundPosition: 'center' }}></div>
            </div>
        </div>
      </div>
      {parent!=='dock'?(<div className="title mt-2.5 min-h-4 whitespace-nowrap w-full px-5 box-border overflow-hidden text-ellipsis text-center ">{name}</div>):null}
      </a>
